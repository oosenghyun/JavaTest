...
public class Ex32_Array {
	...
	private static void m12() {
		...
		for(int i = 0 ; i <numbs.length-1 ; i++) {//0,1,2,3 >cycle
			
			for(int j =0; j < numbs.length -1 - i; j++) {//비교 후 스왑
				if(numbs[j] > numbs[j+1]) {//오름차순 // 앞의 숫자가 더 크면,
	...
	private static void m9() {
		...
		copy = Arrays.copyOfRange(nums, 0, nums.length); //nums배열의 처음부터 끝까지 복사
	...
	private static void m8() {
		...
		nums[0] = 1000; //주소값을 복사해준다.
		
		System.out.println("nums[0]" + nums[0]);
		System.out.println("nums[0]" + copy[0]);
	}
	...
	private static void m7() {
		...
		//원본 수정
		//***nums와 copy는 같은 주소값을 가지고 같은 배열을 가리킨다> 둘 중 하나를 조작하면 나머지 배열에도
		//영향을 받는다. Side Effect가 발생한다. > 조심
		nums[0] = 1000;
	...
	private static void m5() {
		...
		//***배열은 반드시 처음부터 사용하는 최대 길이 미리 예측 > 나중에 수정 불가능
