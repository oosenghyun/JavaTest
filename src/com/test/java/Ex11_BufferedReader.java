...
public class Ex11_BufferedReader {
	public static void main(String[] args) throws Exception{
		...
		//reeadLine() : 입력 버퍼에서 처음부터 ~ 엔터(\r\n)을 만나기 전까지 모두 읽어오는 메소드
		//-엔터는 자동으로 삭제
	    ...
	    /*
	       ...
	       에러, Error
	        ...
	        1. 컴파일 에러
	            -컴파일 작업 중에 발생하는 에러
	            -컴파일러가 발견(java.exe) > 번역하다가 > 문법이 틀려서
	            -오류 발생> 컴파일 작업 중단 > 프로그램 생성 불가 > 반드시 해결
	            -이클립스 연동 > 빨간 밑줄
	        ...
	        2. 런타임 에러
	             ...
	             -컴파일 작업 중에는 에러가 없음 > 실행 중에 발생
	             -문법에는 오류가 없는데 다른 원인으로 발생하는 에러
	             -예외 (Exception)
	             -난이도 중간 > 발견o,발견x >복불복 
	             
	             -충분히 사전에 테스트
	             -게임 출시 > 클로즈드 베타 > 오픈 알파, 오픈 베타
